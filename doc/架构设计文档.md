...
由于学生端基于 Flutter Web（PWA），且当前无本地 Flutter SDK 重编译条件，语音对话的**持续模式**在 `index.html` 的 JS 层实现，架构如下：

```
┌──────────────────────────────────────────────────────┐
│ index.html (JS Bridge)                               │
│  ┌─────────────┐  ┌──────────────┐  ┌──────────────┐│
│  │ STT Bridge   │  │ TTS Bridge   │  │ Voice Chat   ││
│  │ (Web Speech) │  │ (Synthesis)  │  │ Controller   ││
│  └──────┬──────┘  └──────┬──────┘  └──────┬───────┘│
│         │                │                │         │
│         │ onresult       │ ttsSpeak()     │ XHR     │
│         ▼                ▼                ▼         │
│  _stt.finalText    speechSynthesis   /chat/message  │
│  _stt.interimText                     (direct API)  │
│         │                                           │
│  silenceTimer (200ms interval)                       │
│  SILENCE_MS = 1500                                   │
│  holdPatterns = ["还没说完", ...]                     │
└──────────────────────────────────────────────────────┘
         │                              ▲
         │ sttPoll() (Dart ← JS)        │ XHR interceptor
         ▼                              │ captures token/sessionId
┌─────────────────────┐        ┌────────┴──────────┐
│ Flutter Dart App     │        │ FastAPI Backend    │
│ (compiled main.dart) │        │ /api/v1/chat/msg   │
└─────────────────────┘        └───────────────────┘
```

- **麦克风控制**：**默认自动开启**，进入聊天界面后延迟800ms自动启动语音识别，无需手动点击
- **语音识别常开模式**：语音识别持续运行，AI回复时不关闭语音识别，保持连续对话体验
- **静默检测**：JS `setInterval` 每 200ms 检查 `lastResultMs`，超过 **1.0s** 触发 `_vcOnSilence`自动发送
- **防止重复触发**：静默检测添加时间窗口判断（300ms），避免同一次静默重复触发导致倒计时叠加
- **直接 API 调用**：JS 捕获 Dart XHR 中的 `Authorization` 头和 `session_id`，在语音模式下由 JS 直接调用 `/chat/message`，绕过 Dart 编译限制。
- **"还没说完"**：本地关键词匹配，命中后取消请求、停 TTS、累积文本。
- **TTS 控制**：移除 XHR 拦截器中的重复 TTS 调用，统一由 Flutter Dart 层控制语音播放，避免冲突

### 9.2 管理后台用户管理

管理后台新增**用户管理**模块，架构层面：

- **后端**：`admin.py` 新增 `GET /admin/users`、`PUT /admin/users/{id}/reset-password`、`PUT /admin/users/{id}/toggle-active` 三个端点，均需管理员 JWT。
- **数据库**：`users` 表新增 `plain_password` 字段（可空），注册/登录/改密时同步写入。
- **前端**：`admin.html` 新增"用户管理" tab，表格展示用户列表（含明文密码），支持重置密码和禁用操作。

### 9.3 Profile 缓存防重复

- JS 层 XHR 拦截器在 `open()` 阶段注册 `load` 事件监听（先于 Dio），实现：
  - 成功响应缓存到 `localStorage._profile_cache`
  - 失败响应用缓存数据覆盖 XHR 属性（`Object.defineProperty`），防止已设置用户再次看到初始化界面
