...
「学习指认AI」是一套面向 K12 的 **语音优先、多模态交互** 的智能学习平台，通过 **USB 摄像头 OCR、指尖指认识别、实时音视频与 AI 辅导** 串联「做题—纠错—讲解—错题沉淀—家长介入」闭环。
...
| **家长端（Android / Web）** | Flutter（Web/Android），与 App 同域 **8000** | **远程教学干预**、权限与策略；**错题本** 生成、审阅、再生成 |
| **官网** | 静态 HTML，**80** 端口 | 品牌与产品介绍；内置 **管理后台** 路径 `/admin` |
...
| 家长端 | Flutter **Web** / **Android** | 远程干预、错题本与家长权限管理 |
...
| 指尖检测 | **MediaPipe**（**浏览器端**，免费） | 指认坐标采集，与题目区域映射结合 |
