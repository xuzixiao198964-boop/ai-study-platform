...
def main():
    ...
    # 3. 上传修复的speech_service.dart
    print("\n[3] Uploading fixed speech_service.dart...")
    if not upload_file(sftp,
                      "D:/work/ai-study-platform/mobile/lib/services/speech_service.dart",
                      "/opt/ai-study-mobile/lib/services/speech_service.dart"):
        print("Failed to upload speech_service.dart")
        sftp.close()
        client.close()
        return False
    ...
    exec_cmd(client, "systemctl status nginx --no-pager -l | head -20", "Nginx status")
    exec_cmd(client, "systemctl status ai-study --no-pager -l | head -20", "Backend status")
