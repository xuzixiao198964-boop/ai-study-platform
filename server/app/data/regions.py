"""
中国省市区三级行政区划数据（精简版，覆盖全国 31 省级 + 主要地级市 + 市辖区）。
数据结构: { "省名": { "市名": ["区名", ...], ... }, ... }
前端通过三级联动接口获取下拉选项。
"""
...
REGIONS: dict[str, dict[str, list[str]]] = {
    ...
    "江苏省": {
        ...
        "苏州市": ["虎丘区", "吴中区", "相城区", "姑苏区", "吴江区", "昆山市", "张家港市", "常熟市", "太仓市"],
        "南通市": ["崇川区", "通州区", "海门区"],
        "连云港市": ["连云区", "海州区", "赣榆区"],
...
def get_cities(province: str) -> list[str]:
    return list(REGIONS.get(province, {}).keys())


def get_districts(province: str, city: str) -> list[str]:
    return REGIONS.get(province, {}).get(city, [])
